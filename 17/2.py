...
def generate_space(tab):
    return [[[['.' for _ in range(0, len(tab[0])+6*2)] for _ in range(0, len(tab)+6*2)] for _ in range(0, 1+6*2)] for _ in range(0, 1+6*2)]


def insert_input(current_space, cycles):
    for i in range(0, len(tab)):
        for j in range(0, len(tab[0])):
            current_space[cycles][cycles][cycles + i][cycles + j] = tab[i][j]
            ...
            for y in range(0, len(current_space[0][0])):
                for x in range(0, len(current_space[0][0][0])):
                    ...
                    for neighbor in itertools.product([-1, 0, 1], repeat=4):
                        if neighbor != (0, 0, 0, 0):
                            if check_neighbor(get_neighbor_coords(w, z, y, x, neighbor=neighbor),
                                              current_space=current_space):
                                active_counter += 1
                    new_space[w][z][y][x] = update_status(current_space[w][z][y][x], active_counter)
